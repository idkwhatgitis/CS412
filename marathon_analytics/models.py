...
class Result(models.Model):
    ...
    def get_runner_passed_by(self):
        start_late = Result.objects.filter(start_time_of_day__gt = self.start_time_of_day)
        passed = start_late.filter(finish_time_of_day__lt=self.finish_time_of_day)
        return len(passed)

def load_data():
    '''loading data records from csv files'''

    '''delete all records if needed'''

    Result.objects.all().delete() #if we want to strat with an EMPTY DB
    filename = '../2023_chicago_results.csv'
    f = open(filename)
    headers = f.readline() #discard first line which includes the headers

    #line = f.readline().strip() #strip remove the space at the end
